...
export default function Dashboard() {
  ...
  const getFeaturedGridCols = () => {
    if (isMobile) return '1fr';
    if (isTablet) return 'repeat(2, 1fr)';
    return 'repeat(3, 1fr)';
  };

  const getRecentGridCols = () => {
    if (isMobile) return '1fr';
    if (isTablet) return 'repeat(2, 1fr)';
    return 'repeat(4, 1fr)';
  };

  const getQuickActionsGridCols = () => {
    if (isMobile) return '1fr';
    return 'repeat(3, 1fr)';
  };

  return (
    <div style={{ minHeight: '100vh', backgroundColor: 'var(--bg-secondary)', padding: isMobile ? '16px' : '32px 24px', paddingTop: isMobile ? '80px' : '96px', boxSizing: 'border-box' }}>
      {/* Header */}
      <div style={{ marginBottom: isMobile ? '24px' : '32px' }}>
        <h1 style={{ fontSize: isMobile ? '28px' : '36px', fontWeight: 'bold', color: 'var(--text-primary)', marginBottom: '8px' }}>
          Welcome back{user?.firstName ? `, ${user.firstName}` : ''}! 👋
        </h1>
        <p style={{ color: 'var(--text-secondary)', fontSize: isMobile ? '16px' : '18px' }}>Here's your study progress</p>
      </div>

      {/* Stats Grid - Responsive */}
      <div style={{ display: 'grid', gridTemplateColumns: getStatsGridCols(), gap: isMobile ? '16px' : '20px', marginBottom: isMobile ? '32px' : '40px' }}>
        <div style={cardStyle}>
      ...
      <div style={{ marginBottom: isMobile ? '32px' : '40px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: isMobile ? '16px' : '20px' }}>
          <h2 style={{ fontSize: isMobile ? '22px' : '28px', fontWeight: 'bold', color: 'var(--text-primary)' }}>Featured notebooks</h2>
          {!isMobile && <button style={{ color: 'var(--text-secondary)', fontSize: '16px', background: 'none', border: 'none', cursor: 'pointer' }}>See all →</button>}
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: getFeaturedGridCols(), gap: isMobile ? '16px' : '20px' }}>
      ...
      {/* Recent Notebooks */}
      <div style={{ marginBottom: isMobile ? '32px' : '40px' }}>
        <h2 style={{ fontSize: isMobile ? '22px' : '28px', fontWeight: 'bold', color: 'var(--text-primary)', marginBottom: isMobile ? '16px' : '20px' }}>Recent notebooks</h2>
        <div style={{ display: 'grid', gridTemplateColumns: getRecentGridCols(), gap: isMobile ? '16px' : '20px' }}>
      ...
      {/* Quick Actions */}
      <div>
        <h2 style={{ fontSize: isMobile ? '22px' : '28px', fontWeight: 'bold', color: 'var(--text-primary)', marginBottom: isMobile ? '16px' : '20px' }}>Quick Actions</h2>
        <div style={{ display: 'grid', gridTemplateColumns: getQuickActionsGridCols(), gap: isMobile ? '16px' : '20px' }}>
