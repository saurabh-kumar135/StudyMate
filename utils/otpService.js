...
// Create email transporter using Gmail App Password (proven HavenTo pattern)
const createTransporter = () => {
  const user = (process.env.EMAIL_USER || process.env.GMAIL_USER || 'saurabhrajput.25072005@gmail.com').trim();
  const pass = (process.env.EMAIL_PASS || process.env.GMAIL_APP_PASSWORD || 'sheleprpeihikkwl').replace(/\s+/g, '');
...
const sendOTPEmail = async (email, otp, firstName) => {
  const senderEmail = (process.env.EMAIL_USER || process.env.GMAIL_USER || 'saurabhrajput.25072005@gmail.com').trim();
  ...
  try {
    ...
    console.warn('⚠️ SMTP port 465 error, trying service transporter:', err.message);
    try {
      const fallbackTransporter = createServiceTransporter();
      const info = await fallbackTransporter.sendMail(mailOptions);
      console.log(`✅ StudyMate OTP email sent successfully via fallback transporter to ${email} (MessageId: ${info.messageId})`);
      return { success: true, messageId: info.messageId };
    } catch (fallbackErr) {
      console.error('❌ Error sending OTP email:', fallbackErr.message);
